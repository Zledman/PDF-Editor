...
import { exportAsJPG } from '../services/fileExport';
...
export default function PdfToJpgTool({ initialFiles = [], onClose }) {
    ...
    const run = async () => {
        ...
        setIsRunning(true);
        try {
            const result = await exportAsJPG(pdfDoc, [], [], []);

            if (result.pages.length === 1) {
                // Single page - download directly
                const response = await fetch(result.pages[0].dataUrl);
                ...
                for (const page of result.pages) {
                    ...
                    zip.file(`${safeFilename(filename)}_page${page.pageNum}.jpg`, arrayBuffer);
                }
                const zipBlob = await zip.generateAsync({ type: 'blob' });
                downloadBlob(zipBlob, `${safeFilename(filename)}_images.zip`);
            }
        ...
        <div>
            <div style={{ display: 'flex', gap: 12, alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
                <div style={{ color: '#333', fontWeight: 800, fontSize: 18 }}>
                    {t('landingPage.tools.pdfToJpg', 'PDF to JPG')}
                </div>
            ...
            <div style={{ marginBottom: 12, padding: 12, borderRadius: 12, border: '1px solid #eee', background: '#fafafa' }}>
                ...
                {file ? (
                    ...
                    <div style={{ color: '#666' }}>{t('tools.convertFrom.jpgHint', 'Select a PDF to convert to JPG images.')}</div>
                )}
            </div>

            <div style={{ display: 'flex', gap: 10, alignItems: 'center', marginBottom: 12 }}>
                <label style={{ fontSize: 13, fontWeight: 650, color: '#444' }}>{t('downloadModal.filename', 'Filename')}</label>
                <input type="text" value={filename} onChange={(e) => setFilename(e.target.value)} placeholder="images" style={{ flex: 1, padding: '10px 12px', borderRadius: 10, border: '1px solid #ddd', fontSize: 14 }} />
            </div>
            ...
            <div style={{ display: 'flex', gap: 12, justifyContent: 'flex-end', marginTop: 14 }}>
                ...
                <button type="button" onClick={run} disabled={!canRun} style={{ padding: '10px 16px', borderRadius: 10, border: 'none', background: canRun ? '#F4D35E' : '#bbb', color: canRun ? '#333' : '#fff', cursor: canRun ? 'pointer' : 'not-allowed', fontWeight: 800, opacity: canRun ? 1 : 0.7 }}>
