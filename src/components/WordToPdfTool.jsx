...
export default function WordToPdfTool({ initialFiles = [], onClose }) {
    ...
    const serverUrl = import.meta.env.VITE_CONVERT_SERVER_URL || 'http://localhost:8082/convert/word-to-pdf';

    useEffect(() => {
        const first = initialFiles?.[0];
        if (!first?.buffer || file) return;
        setFile({ name: first.name || 'document.docx', size: first.size || first.buffer.byteLength, buffer: first.buffer });
    ...
    const onPick = async (f) => {
        ...
        try {
            ...
            setFile({ name: f.name || 'document.docx', size: f.size || buf.byteLength, buffer: buf });
    ...
    const run = async () => {
        ...
        try {
            ...
            form.append('file', new Blob([file.buffer], { type: 'application/vnd.openxmlformats-officedocument.wordprocessingml.document' }), file.name);
        ...
        <div>
            <div style={{ display: 'flex', gap: 12, alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
                <div style={{ color: '#333', fontWeight: 800, fontSize: 18 }}>
                    {t('landingPage.tools.wordToPdf', 'WORD to PDF')}
                </div>
            ...
            <div style={{ marginBottom: 12, padding: 12, borderRadius: 12, border: '1px solid #eee', background: '#fafafa' }}>
                ...
                {file ? (
                    ...
                    <div style={{ color: '#666' }}>{t('tools.convert.wordHint', 'Select a Word document (.doc, .docx) to convert to PDF.')}</div>
                )}
            </div>
            ...
            <div style={{ display: 'flex', gap: 12, justifyContent: 'flex-end', marginTop: 14 }}>
                ...
                <button
                    ...
                    onClick={run}
                    disabled={!canRun}
                    style={{ padding: '10px 16px', borderRadius: 10, border: 'none', background: canRun ? '#4D8AE6' : '#bbb', color: '#fff', cursor: canRun ? 'pointer' : 'not-allowed', fontWeight: 800, opacity: canRun ? 1 : 0.7 }}
                >
