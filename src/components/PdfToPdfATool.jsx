...
function safeFilename(name) {
    const trimmed = String(name || '').trim() || 'document';
...
export default function PdfToPdfATool({ initialFiles = [], onClose }) {
    ...
    const serverUrl = import.meta.env.VITE_CONVERT_SERVER_URL?.replace('word-to-pdf', 'pdf-to-pdfa') || 'http://localhost:8082/convert/pdf-to-pdfa';
    ...
    const run = async () => {
        ...
        try {
            ...
            if (!resp.ok) {
                ...
                throw new Error(`${t('tools.convertFrom.error', 'Conversion failed')} (${resp.status})${bodyText ? `: ${bodyText.slice(0, 300)}` : ''}`);
            }
        ...
        <div>
            <div style={{ display: 'flex', gap: 12, alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
                <div style={{ color: '#333', fontWeight: 800, fontSize: 18 }}>
                    {t('landingPage.tools.pdfToPdfA', 'PDF to PDF/A')}
                </div>
                <div style={{ display: 'flex', gap: 8 }}>
                    ...
                    <button type="button" onClick={() => { setFile(null); setSuccessMsg(''); setErrMsg(''); }} disabled={!file || isRunning} style={{ padding: '10px 12px', borderRadius: 10, border: '1px solid #ddd', background: !file || isRunning ? '#f3f3f3' : '#fff', cursor: !file || isRunning ? 'not-allowed' : 'pointer', fontWeight: 650, opacity: !file || isRunning ? 0.6 : 1 }}>
            ...
            <div style={{ marginBottom: 12, padding: 12, borderRadius: 12, border: '1px solid #eee', background: '#fafafa' }}>
                ...
                {file ? (
                    ...
                    <div style={{ color: '#666' }}>{t('tools.convertFrom.pdfaHint', 'Select a PDF to convert to PDF/A format for archiving.')}</div>
                )}
                <div style={{ marginTop: 8, fontSize: 12, color: '#666' }}>
                    {t('tools.convertFrom.pdfaDescription', 'PDF/A is an ISO-standardized format for long-term digital document preservation.')}
                </div>
            </div>

            <div style={{ display: 'flex', gap: 10, alignItems: 'center', marginBottom: 12 }}>
                <label style={{ fontSize: 13, fontWeight: 650, color: '#444' }}>{t('downloadModal.filename', 'Filename')}</label>
                <input type="text" value={filename} onChange={(e) => setFilename(e.target.value)} placeholder="document_pdfa" style={{ flex: 1, padding: '10px 12px', borderRadius: 10, border: '1px solid #ddd', fontSize: 14 }} />
            </div>
            ...
            <div style={{ display: 'flex', gap: 12, justifyContent: 'flex-end', marginTop: 14 }}>
                ...
                <button type="button" onClick={run} disabled={!canRun} style={{ padding: '10px 16px', borderRadius: 10, border: 'none', background: canRun ? '#7B7BEA' : '#bbb', color: '#fff', cursor: canRun ? 'pointer' : 'not-allowed', fontWeight: 800, opacity: canRun ? 1 : 0.7 }}>
