...
import { exportAsPPTX } from '../services/fileExport';
...
export default function PdfToPowerpointTool({ initialFiles = [], onClose }) {
    ...
    const loadPdf = async (buffer, name) => {
        try {
            ...
            const stem = (name || 'presentation').replace(/\.[^.]+$/, '');
            setFilename(stem);
        } catch (e) {
    ...
    const run = async () => {
        ...
        setIsRunning(true);
        try {
            const result = await exportAsPPTX(pdfDoc, [], [], []);
        ...
        <div>
            <div style={{ display: 'flex', gap: 12, alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
                <div style={{ color: '#333', fontWeight: 800, fontSize: 18 }}>
                    {t('landingPage.tools.pdfToPowerpoint', 'PDF to POWERPOINT')}
                </div>
            ...
            <div style={{ marginBottom: 12, padding: 12, borderRadius: 12, border: '1px solid #eee', background: '#fafafa' }}>
                ...
                {file ? (
                    ...
                    <div style={{ color: '#666' }}>{t('tools.convertFrom.pptHint', 'Select a PDF to convert to PowerPoint.')}</div>
                )}
            </div>

            <div style={{ display: 'flex', gap: 10, alignItems: 'center', marginBottom: 12 }}>
                <label style={{ fontSize: 13, fontWeight: 650, color: '#444' }}>{t('downloadModal.filename', 'Filename')}</label>
                <input type="text" value={filename} onChange={(e) => setFilename(e.target.value)} placeholder="presentation" style={{ flex: 1, padding: '10px 12px', borderRadius: 10, border: '1px solid #ddd', fontSize: 14 }} />
            </div>
            ...
            <div style={{ display: 'flex', gap: 12, justifyContent: 'flex-end', marginTop: 14 }}>
                ...
                <button type="button" onClick={run} disabled={!canRun} style={{ padding: '10px 16px', borderRadius: 10, border: 'none', background: canRun ? '#E76F51' : '#bbb', color: '#fff', cursor: canRun ? 'pointer' : 'not-allowed', fontWeight: 800, opacity: canRun ? 1 : 0.7 }}>
