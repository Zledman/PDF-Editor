...
import { exportAsWord } from '../services/fileExport';
...
export default function PdfToWordTool({ initialFiles = [], onClose }) {
    ...
    const run = async () => {
        ...
        setIsRunning(true);
        try {
            const result = await exportAsWord(pdfDoc, [], [], []);
        ...
        <div>
            <div style={{ display: 'flex', gap: 12, alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
                <div style={{ color: '#333', fontWeight: 800, fontSize: 18 }}>
                    {t('landingPage.tools.pdfToWord', 'PDF to WORD')}
                </div>
            ...
            <div style={{ marginBottom: 12, padding: 12, borderRadius: 12, border: '1px solid #eee', background: '#fafafa' }}>
                ...
                {file ? (
                    ...
                    <div style={{ color: '#666' }}>{t('tools.convertFrom.wordHint', 'Select a PDF to convert to Word document.')}</div>
                )}
            </div>

            <div style={{ display: 'flex', gap: 10, alignItems: 'center', marginBottom: 12 }}>
                <label style={{ fontSize: 13, fontWeight: 650, color: '#444' }}>{t('downloadModal.filename', 'Filename')}</label>
                <input type="text" value={filename} onChange={(e) => setFilename(e.target.value)} placeholder="document" style={{ flex: 1, padding: '10px 12px', borderRadius: 10, border: '1px solid #ddd', fontSize: 14 }} />
            </div>
