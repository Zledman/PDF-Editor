...
function safeFilename(name) {
    const trimmed = String(name || '').trim() || 'ocr';
...
const LANGUAGES = [
    ...
    { key: 'swe+eng', label: 'Svenska + English' }
];

export default function OcrPdfTool({ initialFiles = [], onClose }) {
    const { t } = useTranslation();
    const fileInputRef = useRef(null);

    const [file, setFile] = useState(null);
    const [filename, setFilename] = useState('ocr_output');
    const [lang, setLang] = useState('swe+eng');
    const [isRunning, setIsRunning] = useState(false);
    const [errMsg, setErrMsg] = useState('');
    const [successMsg, setSuccessMsg] = useState('');

    const ocrServerUrl = import.meta.env.VITE_OCR_SERVER_URL || 'http://localhost:8082/ocr-pdf';
    ...
    const run = async () => {
        ...
        try {
            ...
            const url = `${ocrServerUrl}?lang=${encodeURIComponent(lang)}`;
            const resp = await fetch(url, { method: 'POST', body: form });
            if (!resp.ok) {
                const bodyText = await resp.text().catch(() => '');
                const short = bodyText ? bodyText.slice(0, 300) : '';
                throw new Error(`${t('tools.ocr.error', 'OCR failed')} (${resp.status})${short ? `: ${short}` : ''}`);
            }

            const blob = await resp.blob();
            downloadBlob(blob, `${safeFilename(filename)}.pdf`);
            setSuccessMsg(t('tools.ocr.success', 'PDF is now searchable!'));
        } catch (e) {
            setErrMsg((e && e.message) || t('tools.ocr.error', 'OCR failed'));
        } finally {
            setIsRunning(false);
        }
    };
        ...
        <div>
            <div style={{ display: 'flex', gap: 12, alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
                <div style={{ color: '#333', fontWeight: 800, fontSize: 18 }}>
                    {t('landingPage.tools.ocrPdf', 'OCR PDF')}
                </div>
            ...
            <div style={{ marginBottom: 12, padding: 12, borderRadius: 12, border: '1px solid #eee', background: '#fafafa' }}>
                ...
                {file ? (
                    ...
                    <div style={{ color: '#666' }}>{t('tools.ocr.hint', 'Choose a scanned PDF to make searchable.')}</div>
                )}
                <div style={{ marginTop: 8, fontSize: 12, color: '#666' }}>
                    {t('tools.ocr.description', 'Upload a scanned PDF and we will recognize the text. The result is a searchable PDF where you can select and copy text.')}
                </div>
            </div>

            <div style={{ display: 'flex', gap: 10, alignItems: 'center', marginBottom: 12 }}>
                <label style={{ fontSize: 13, fontWeight: 650, color: '#444' }}>
                    {t('tools.ocr.language', 'Language')}
                </label>
                <select
                    value={lang}
            ...
            <div style={{ display: 'flex', gap: 12, justifyContent: 'flex-end', marginTop: 14 }}>
                ...
                >
                    {isRunning ? t('loading.processing', 'Processing...') : t('tools.ocr.run', 'Run OCR')}
                </button>
            </div>
        </div>
    );
