...
public class ReplaceController {
    ...
    try (PDDocument doc = PDDocument.load(pdfBytes)) {
      if (req != null && req.replacements != null) {
        for (ReplaceRequest.Replacement rp : req.replacements) {
          if (rp == null || rp.bbox == null) continue;
          int pageIndex = Math.max(0, (rp.page > 0 ? rp.page - 1 : 0));
          if (pageIndex >= doc.getNumberOfPages()) continue;
          PDPage page = doc.getPage(pageIndex);

          // Best-effort: ta bort text (Tj/TJ) vars textposition hamnar inom bbox.
    ...
    for (Object tok : tokens) {
      ...
      switch (name) {
        ...
        case "Tj" -> {
          ...
          if (str == null || !isInBBox(page, textMatrix, fontSize, rp)) {
            flush(newTokens, args, op);
          } else {
        ...
        case "TJ" -> {
          ...
          if (!isInBBox(page, textMatrix, fontSize, rp)) {
            flush(newTokens, args, op);
          } else {
            args.clear();
          }
   ...
   */
  private static boolean isInBBox(PDPage page, Matrix textMatrix, float fontSize, ReplaceRequest.Replacement rp) {
    ...
    // Bästa-gissning: testa textens startpunkt mot bbox
    return x >= bx && x <= bx + bw && y >= by && y <= by + bh;
  }

  private static float[] parseHex(String hex) {
    if (hex == null) return new float[]{0f, 0f, 0f};
    String s = hex.trim();
    if (s.startsWith("#")) s = s.substring(1);
    if (s.length() != 6) return new float[]{0f, 0f, 0f};
    try {
  ...
  private static void writeNewText(PDDocument doc, PDPage page, ReplaceRequest.Replacement rp) throws IOException {
    ...
    // PDFBox kan inte "showText" med kontrolltecken (t.ex. \n). Hantera multi-line explicit.
    String text = rp.text != null ? rp.text : "";
    text = text.replace("\r\n", "\n").replace("\r", "\n");
    // Byt ut övriga kontrolltecken (tab, etc) till mellanslag för att undvika encode-crash.
    ...
    try (PDPageContentStream cs = new PDPageContentStream(doc, page, PDPageContentStream.AppendMode.APPEND, true, true)) {
      ...
      try {
        ...
        // Säkerställ endText även om showText kastar exception, så vi slipper varningen i loggen.
        cs.endText();
      }
    }
  }
